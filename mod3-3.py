...
for vendor, data in cars.items():
    vendor_2018_qty = 0
    for model, qty in data.items():
        vendor_2018_qty += qty['sales']['2018']
        if vendor_2018_qty > total_2018_qty:
            total_2018_qty = vendor_2018_qty
            answer2 = vendor
        print("Samochód {} sprzedał się w liczbie {} egzemplarzy".format(model, qty['sales']['2017']))
        if qty['sales']['2017'] > temp_qty:
            temp_qty = qty['sales']['2017']
            answer1 = model
        if (qty['sales']['2016'] == 0) and (not qty['sales']['2017'] == 0):
